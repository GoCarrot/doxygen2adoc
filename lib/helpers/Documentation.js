...
const stripNewlineElements = (arr) => {
...
export default class Documentation {
  ...
  #parsePara(para) {
    return para.reduce((arr, para) => {
      if (para.computeroutput) {
        assert.strictEqual(para.computeroutput.length, 1,
            '#parsePara computeroutput element length > 1');

        // TODO: This is hard-coded asciidoc formatting
        return arr.concat(['``' + para.computeroutput[0].$text + '``']);
      } else if (para.simplesect) {
        return arr.concat(this.#parseSimpleSect(stripNewlineElements(para.simplesect), para[':@']?.$kind));
      } else if (para.xrefsect) {
        // console.log(para);
        return arr.concat(this.#parseXrefSect(para.xrefsect));
      } else if (para.parameterlist) {
        return arr.concat(this.#parseParameterList(stripNewlineElements(para.parameterlist)));
      } else if (para.itemizedlist) {
        return arr.concat(this.#parseItemizedList(stripNewlineElements(para.itemizedlist)));
      } else if (para.ref) {
        // TODO: This is where we turn ref into link
        assert.strictEqual(para.ref.length, 1);
        return arr.concat([para.ref[0].$text]);
      } else if (para.$text) {
        return arr.concat([para.$text]);
      } else {
        console.log(para);
   ...
   * @param {String} kind The kind of simplesect element
   * @return {Array} Array of elements to merge into description.
   */
  #parseSimpleSect(simplesect, kind) {
    return simplesect.reduce((arr, simplesect) => {
      switch (kind) {
        case 'return':
          this.return = this.#parsePara(simplesect.para).join('\n');
          return arr;
        case 'note':
          this.note.push(this.#parsePara(simplesect.para).join(''));
          return arr;
        case 'warning':
          this.warning.push(this.#parsePara(simplesect.para).join(''));
          return arr;
        case undefined:
          break;
  ...
  #parseXrefSect(xrefsect) {
    ...
    if (xreftitle === 'Deprecated') {
      ...
      const xrefdescription = stripNewlineElements(xrefsect[1].xrefdescription);
      ...
      this.deprecatedDocs =
        this.deprecatedDocs.concat(xrefdescription.reduce((arr, xrefdescription) => {
          ...
          return arr.concat(this.#parsePara(xrefdescription.para));
        }, []));
    } else {
  ...
  #parseParameterList(parameterlist) {
    return parameterlist.reduce((arr, parameterlist) => {
      ...
      const parameteritem = stripNewlineElements(parameterlist.parameteritem);
      ...
      const parameternamelist = stripNewlineElements(parameteritem[0].parameternamelist)[0];
      const parameterdescription = stripNewlineElements(parameteritem[1].parameterdescription)[0];
  ...
  #parseItemizedList(itemizedlist) {
    return itemizedlist.reduce((arr, itemizedlist) => {
      return arr.concat(stripNewlineElements(itemizedlist.listitem).reduce((arr, listitem) => {
        // TODO: This is hard-coded asciidoc formatting
        const parsedListItem = this.#parsePara(listitem.para);
        return arr.concat([`\n* ${parsedListItem.join('')}`]);
      }, []));
    }, []);
  }
