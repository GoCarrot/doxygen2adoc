...
export default class Class extends Compound {
  ...
  constructor(xmlDef, createFn, language) {
    ...
    if (xmlDef.basecompoundref) {
      this.basecompoundref = Array.isArray(xmlDef.basecompoundref) ? xmlDef.basecompoundref : [xmlDef.basecompoundref];
    }
  ...
  finalize() {
    this.inheritsFrom = this.basecompoundref.reduce((arr, basecompoundref) => {
